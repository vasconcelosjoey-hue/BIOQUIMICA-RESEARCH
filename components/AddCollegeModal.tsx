...
const AddCollegeModal: React.FC<AddCollegeModalProps> = ({ isOpen, onClose, onAdd }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!errorMessage) {
      setFormData({ name: '', city: '', state: '', type: 'Pública Federal', phone: '', website: '', courses: '' });
      onClose();
    } else {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-[2rem] w-full max-w-xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300">
        <div className="px-8 py-6 bg-gradient-to-r from-teal-800 to-teal-900 text-white flex justify-between items-center">
          <div>
            <h2 className="text-2xl font-black">Nova Pesquisa</h2>
            <p className="text-teal-300 text-xs font-bold uppercase tracking-widest mt-1">Expanda o diretório nacional</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-5">
          {error && (
            <div className="bg-red-50 border border-red-100 p-4 rounded-2xl text-red-600 text-sm flex items-center gap-3 animate-shake">
          ...
          <div className="flex gap-4 pt-4">
            <button
              type="submit"
              className="flex-1 py-4 bg-teal-600 text-white font-black rounded-2xl hover:bg-teal-700 shadow-xl shadow-teal-200 transition-all transform active:scale-95"
            >
              SALVAR NO DIRETÓRIO
            </button>
          </div>
